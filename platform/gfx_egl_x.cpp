...
namespace cee {
namespace platform {
	X11GraphicsContext::X11GraphicsContext(GfxContextType ctxType, Logger logger)
	 : GraphicsContext(ctxType, logger), m_Display(nullptr), m_Window(0), m_Screen(0), m_WmDeleteAtom(0),
	...
	void X11GraphicsContext::Init()
	{
		...
		if (m_Display == 0) {
			error(logger(), "Failed to open X display");
			throw core::InternalError("Failed to open X display");
		}

		m_Screen = DefaultScreen(m_Display);

		if (!gladLoaderLoadEGL(EGL_NO_DISPLAY)) {
			error(logger(), "EGL loader error");
			throw core::InternalError("Failed to load egl function pointers");
		}
		...
		XStoreName(m_Display, m_Window, "ceeMPPM");
		...
		if (m_EglDisplay == EGL_NO_DISPLAY) {
			error(logger(), "Unable to get EGLDisplay from X");
			XDestroyWindow(m_Display, m_Window);
			throw core::InternalError("Failed to get EGL display from X");
		}
		...
		if (!gladLoaderLoadEGL(m_EglDisplay)) {
			error(logger(), "Failed to load EGL");
			XDestroyWindow(m_Display, m_Window);
			throw core::InternalError("Failed to load egl function pointers with display");
		}
		...
		if (m_EglSurface == EGL_NO_SURFACE) {
			error(logger(), "Failed to create EGL surface");
			XDestroyWindow(m_Display, m_Window);
		...
		if (m_EglContext == EGL_NO_CONTEXT) {
			error(logger(), "Failed to create EGL context");
			XDestroyWindow(m_Display, m_Window);
		...
		if (!eglMakeCurrent(m_EglDisplay, m_EglSurface, m_EglSurface, m_EglContext)) {
			error(logger(), "Failed to make EGL context current");
			XDestroyWindow(m_Display, m_Window);
			eglDestroyContext(m_EglDisplay, m_EglContext);
			eglDestroySurface(m_EglDisplay, m_EglSurface);
			throw core::InternalError("Failed to make EGL context current");
		}	
		...
		debug(logger(), "\tShading Language Version: {}", this->GetShadingVersionString());
	...
	void X11GraphicsContext::SwapBuffers()
	{
		if (!eglSwapBuffers(m_EglDisplay, m_EglSurface)) {
			warn(logger(), "eglSwapBuffers failed: 0x{:X}", glGetError());
