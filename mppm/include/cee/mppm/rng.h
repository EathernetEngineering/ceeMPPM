...
namespace {
	struct rngImpl {
		inline static void Init(std::optional<uint64_t> seed = {}) {
			...
			if (s_Initialized) {
				CEE_CORE_TRACE("RNG already initialized. Calling this function again will cause a seed reset.");
				CEE_CORE_TRACE("\tCalling this function again will cause a seed reset.");
				CEE_CORE_TRACE("\tTo avoid this message use cee::rng<T>::ResetSeed().");
			}
